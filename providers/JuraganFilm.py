...
class JuraganFilm(object):
    servers = ["http://144.91.95.62"]
    sandbox = "allow-scripts allow-same-origin"

    def search(self, query,page=1):
        headers = {
            "authority": "144.91.95.62",
            "accept": "*/*",
            "accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7",
            "referer": "https://144.91.95.62/",
        ...
        for url in self.servers:
            ...
            try:
                ...
                for article in articles:
                    ...
                    thumb = article.find("img", {"class": "attachment-large"})["src"]
                    link = article.find("a", {"class": "gmr-watch-button"})["href"]
                    detailLink = base64.b64encode(
                        json.dumps({"link": link, "provider": "JuraganFilm"}).encode()
                    ).decode("utf-8")
                    if "/tv/" in link:
                    ...
                    result.append(
                        {
                            "link": "/api/get?link=" + link + "&provider=JuraganFilm",
                            "detail": detailLink,
    ...
    def getMovies(self, url):
        ...
        streamLinks = []
        for link in links:
            _r = base64.b64encode("https://144.91.95.62/".encode())
            _raw = base64.b64decode(link["data-frame"]).decode("utf-8")
            if "uplayer" in _raw:
                _raw += "&r=" + _r.decode("utf-8")
            _url = base64.b64encode(_raw.encode()).decode("utf-8")
            streamLinks.append({"link":_raw,"detail": "/iframe?link=" + _url + "&provider=JuraganFilm", "title": link.text})
    ...
    def getSeries(self, url):
        ...
        for link in links:
            epsLinks.append(
                {
                    "link": "/api/get?link=" + link["href"] + "&provider=JuraganFilm",
                    "detail": "/detail?detail="
                    + base64.b64encode(
                        json.dumps(
                            {"link": link["href"], "provider": "JuraganFilm"}
                        ).encode()
                    ).decode("utf-8"),
                    "title": link.text,
                }
